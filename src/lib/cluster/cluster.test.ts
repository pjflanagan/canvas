...
function isOverlapping(a: TestData, b: TestData): boolean {
  return a
    .split('')
    .map((aValue) => b.split('').some((bValue) => aValue === bValue))
    .some((hasEquality) => hasEquality);
}

function makeOverlappingFunctionTracker(): [
  (a: TestData, b: TestData) => Promise<boolean>,
  () => number,
] {
  let calls = 0;
...
function makeClusterUsingStore(data: TestData[], compare: Comparator<TestData>): TestData[][] {
  ...
  data.forEach((item) => {
    ...
    const [overlappedClusters, nonOverlappedClusters] = partition(clusters, (cluster) =>
      cluster.some((clusteredItem) => compare(item, clusteredItem)),
    );
...
function sortForEqualityCheck(cluster: TestData[][]) {
  return cluster.map((cluster) => cluster.sort()).sort();
}
...
const TESTS: Test[] = [test0 as unknown as Test, test1 as unknown as Test];
...
describe('comparison', () => {
  TESTS.forEach(({ data, description }) => {
    ...
    const [isOverlappingTrackedClusterStore, getCallsClusterStore] =
      makeOverlappingFunctionTracker();
