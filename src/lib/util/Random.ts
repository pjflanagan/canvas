import { ColorMixer, type IColorLegacy } from './ColorMixer';
...
export class Random {
  ...
  static propNumber({ min, max }: RangeProperty, multiplier = 1): number {
    return Math.floor(Random.propFloat({ min, max })) * multiplier;
  }

  static propFloat({ min, max }: RangeProperty, multiplier = 1): number {
    return Random.float(min, max) * multiplier;
  }
